...
/**
 * Runs Maven (mvnw) in a project root. Give it the project root File and "compile" or "test";
 * it builds the right command (Windows vs other OS) and runs it.
 */
public final class Script {
     ...
     */
    public static boolean run(File projectRoot, String action) throws IOException, InterruptedException {
    ...
    private static List<String> goalsFor(String action) {
        switch (action) {
            ...
            case "test":
                return List.of("test");
            default:
                throw new IllegalArgumentException("action must be \"compile\" or \"test\", got: " + action);
        }
    }
}
